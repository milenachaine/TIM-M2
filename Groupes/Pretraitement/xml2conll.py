...
for i in corpus.findall("./doc"):
    ...
    for a in i.findall("./answer"):
        #with open(mydir+"/"+source+"/"+cat+"/"+sub+"/"+ID+"/"+ID+"_a"+str(ID_a)+".conll", "w", encoding="utf-8") as answer:
        for line in a.text.split("\n"):
            os.system('echo "'+line.strip().replace('"','\"')+'" | tree-tagger-french >>'+mydir+"/"+source+"/"+cat+"/"+sub+"/"+ID+"/"+ID+"_a"+str(ID_a)+".conll")
        #print(q.text.strip().replace('"','\"'))
        phrases.write(a.text.strip())
        ID_a+=1
